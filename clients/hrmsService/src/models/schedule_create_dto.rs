/*
 * HrmsService
 *
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: 2.1.2.5401
 * Contact: support@fenix-alliance.com
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct ScheduleCreateDto {
    #[serde(rename = "id", skip_serializing_if = "Option::is_none")]
    pub id: Option<uuid::Uuid>,
    #[serde(rename = "timestamp", skip_serializing_if = "Option::is_none")]
    pub timestamp: Option<String>,
    #[serde(rename = "name")]
    pub name: String,
    #[serde(rename = "description", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub description: Option<Option<String>>,
    #[serde(rename = "disabled", skip_serializing_if = "Option::is_none")]
    pub disabled: Option<bool>,
    #[serde(rename = "sunday", skip_serializing_if = "Option::is_none")]
    pub sunday: Option<bool>,
    #[serde(rename = "monday", skip_serializing_if = "Option::is_none")]
    pub monday: Option<bool>,
    #[serde(rename = "tuesday", skip_serializing_if = "Option::is_none")]
    pub tuesday: Option<bool>,
    #[serde(rename = "wednesday", skip_serializing_if = "Option::is_none")]
    pub wednesday: Option<bool>,
    #[serde(rename = "thursday", skip_serializing_if = "Option::is_none")]
    pub thursday: Option<bool>,
    #[serde(rename = "friday", skip_serializing_if = "Option::is_none")]
    pub friday: Option<bool>,
    #[serde(rename = "saturday", skip_serializing_if = "Option::is_none")]
    pub saturday: Option<bool>,
    #[serde(rename = "uniqueInterval", skip_serializing_if = "Option::is_none")]
    pub unique_interval: Option<bool>,
    #[serde(rename = "is24x7Interval", skip_serializing_if = "Option::is_none")]
    pub is24x7_interval: Option<bool>,
    #[serde(rename = "start", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub start: Option<Option<String>>,
    #[serde(rename = "end", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub end: Option<Option<String>>,
    #[serde(rename = "timezoneId", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub timezone_id: Option<Option<String>>,
    #[serde(rename = "fiscalYearId", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub fiscal_year_id: Option<Option<String>>,
    #[serde(rename = "holidayScheduleId", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub holiday_schedule_id: Option<Option<String>>,
}

impl ScheduleCreateDto {
    pub fn new(name: String) -> ScheduleCreateDto {
        ScheduleCreateDto {
            id: None,
            timestamp: None,
            name,
            description: None,
            disabled: None,
            sunday: None,
            monday: None,
            tuesday: None,
            wednesday: None,
            thursday: None,
            friday: None,
            saturday: None,
            unique_interval: None,
            is24x7_interval: None,
            start: None,
            end: None,
            timezone_id: None,
            fiscal_year_id: None,
            holiday_schedule_id: None,
        }
    }
}

